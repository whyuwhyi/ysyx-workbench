...
static bool is_op(int i) { return get_op_priority(i) >= 0; }

static bool is_binary(int i) { return get_op_priority(i) > 0; }
...
// Placeholder for memory access - will use pmem
  ...
  if (s > e) {
    *success = false;
    ...
    case TK_DEC:
      ...
      return val;
    case TK_HEX:
      sscanf(tokens[s].str, "%x", &val);
      return val;
    case TK_REG:
      return npc_reg_str2val(tokens[s].str + 1, success);
    default:
      *success = false;
      printf("Invalid expression\n");
      assert(0);
    ...
    case TK_BIT_NOT:
      return ~right_val;
    case TK_DEREF:
      return npc_vaddr_read(right_val, 4);
    case TK_NEG:
      return -right_val;
    default:
      *success = false;
...
void test_expr() {
  // Test expression evaluator with simple expressions
  ...
  // Simple arithmetic
  result = expr((char *)"1 + 2", &success);
  printf("1 + 2 = %u (success: %s)\n", result, success ? "true" : "false");

  result = expr((char *)"10 * 5", &success);
  printf("10 * 5 = %u (success: %s)\n", result, success ? "true" : "false");

  result = expr((char *)"0x10", &success);
  printf("0x10 = %u (success: %s)\n", result, success ? "true" : "false");
}
